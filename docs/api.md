...
Endpoint:
POST /api/v1/orders
Требование заголовка: Idempotency-Key: <GUID> — обязателен, защищает от дублирования заказов при сетевых сбоях.
Повторный запрос с тем же ключом и тем же телом возвращает результат исходной операции без повторного создания заказа.
...
Повторный запрос:
Если запрос содержит уже использованный Idempotency-Key и тело запроса соответствует исходному запросу, API возвращает ранее сохранённый HTTP-ответ без повторного выполнения операции создания заказа.

В случае успешного исходного запроса возвращается:
201 Created
с тем же ResponseBody и Location, что и у исходного запроса.

## 5.2 Получение заказа
После создания ресурса клиент может получить его текущее публичное состояние.
...
Значения Pending, Completed и Cancelled являются публичными статусами ресурса заказа.
Клиент взаимодействует только с публичным состоянием ресурса и не зависит от внутреннего механизма выполнения бизнес-процесса.
...
# 9. Инфраструктурные интерфейсы (Health)
...
## 11.1 Пример ошибки валидации (400 Bad Request)
{
...
## 11.2 Пример ошибки оптимистичной блокировки (409 Conflict)
